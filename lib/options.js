...
const { getRcPath, exit, error, validate, createSchema } = require('../utils')
  ...
  if (fs.existsSync(rcPath)) {
    try {
      ...
      error(
        ...
        `~/.emmarc may be corrupted or have syntax errors. ` +
        `Please fix/delete it and re-run emma-cli in manual mode.\n` +
        `(${e.message})`,
      )
      exit(1)
    }
    validate(cachedOptions, schema, () => {
      error(
        `~/.emmarc may be outdated. ` +
        `Please delete it and re-run emma-cli in manual mode.`
      )
    })
    return cachedOptions
  } else {
